...
function PdfViewer({ url, apercuPages }: { url: string; apercuPages?: number | null }) {
  const pages = apercuPages || 3
  // Utiliser Google Docs Viewer pour afficher le PDF sans téléchargement
    ...
    <div className="space-y-3">
      <div className="bg-noir-800 rounded-xl p-3 text-center">
        <p className="text-gold-400 text-xs font-semibold uppercase tracking-wider mb-1">
          Aperçu — {pages} premières pages
        </p>
        <p className="text-noir-500 text-xs">Téléchargement et impression désactivés</p>
      </div>
      <div className="aspect-[3/4] rounded-xl overflow-hidden bg-noir-900 border border-noir-700">
        <iframe
          src={viewerUrl}
      ...
      <div className="bg-noir-900 border border-gold-500/20 rounded-xl p-4 text-center">
        <p className="text-noir-400 text-xs">
          Cet aperçu montre les {pages} premières pages uniquement.
          Achetez pour accéder au document complet.
        </p>
      </div>
    </div>
  )
...
function CloudinaryViewer({ url, type }: { url: string; type: string }) {
  if (type === 'video_youtube' || type === 'formation') {
    // Vidéo Cloudinary
    return (
  ...
  if (type === 'audio') {
    return <AudioPlayer url={url} apercuDuree={30} />
  }
  if (type === 'documentation') {
    return <PdfViewer url={url} apercuPages={3} />
  }
  return (
    <div className="text-center py-8">
      <a href={url} target="_blank" rel="noopener noreferrer" className="btn-gold">
        Ouvrir la ressource
      </a>
    </div>
  )
...
export default function ApercuModal({ ressource, onClose, onBuy }: ApercuModalProps) {
  ...
  const isCloudinary = ressource.apercu_url && ressource.apercu_url.includes('cloudinary')
  const hasAudio = ressource.apercu_url && ressource.type === 'audio'
  const hasPdf = (ressource.apercu_url || ressource.fichier_url) && ressource.type === 'documentation'
    ...
    <AnimatePresence>
      <motion.div
        ...
        <motion.div
          ...
          className="bg-noir-900 border border-noir-700 rounded-2xl w-full max-w-2xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
          ...
          <div className="flex items-center justify-between px-5 py-4 border-b border-noir-800 shrink-0">
            <div>
              ...
              <p className="text-gold-500 text-xs mt-0.5">Aperçu gratuit</p>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-5">
            ...
            {isYoutube && (
              <YoutubeEmbed
                url={ressource.youtube_url!}
                apercuDuree={ressource.apercu_duree}
              />
            )}

            {/* Cloudinary ou URL directe */}
            {!isYoutube && isCloudinary && (
              <CloudinaryViewer url={ressource.apercu_url!} type={ressource.type} />
            )}

            {/* Audio avec URL aperçu */}
            {!isYoutube && !isCloudinary && hasAudio && (
              <AudioPlayer url={ressource.apercu_url!} apercuDuree={ressource.apercu_duree} />
            )}

            {/* PDF */}
            {!isYoutube && !isCloudinary && !hasAudio && hasPdf && (
              <PdfViewer
