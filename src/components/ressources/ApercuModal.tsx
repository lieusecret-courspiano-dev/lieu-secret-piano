...
interface ApercuModalProps {
  ressource: {
    ...
    apercu_duree?: number | null
    apercu_pages?: number | null
...
// ── Aperçu YouTube avec durée limitée ──────────────────────────────────────
function YoutubeEmbed({ url, apercuDuree }: { url: string; apercuDuree?: number | null }) {
  const [blocked, setBlocked] = useState(false)
  const timerRef = useRef<NodeJS.Timeout | null>(null)

  const getYoutubeId = (u: string) => {
    const m = u.match(/(?:youtube\.com\/watch\?v=|youtu\.be\/|youtube\.com\/embed\/)([^&\s?]+)/)
    return m ? m[1] : null
  }
  const videoId = getYoutubeId(url)

  useEffect(() => {
    if (apercuDuree && apercuDuree > 0) {
      timerRef.current = setTimeout(() => setBlocked(true), apercuDuree * 1000)
    }
    ...
    <div className="relative aspect-video rounded-xl overflow-hidden bg-noir-900">
      {!blocked ? (
        ...
        <div className="absolute inset-0 bg-noir-950/95 backdrop-blur-sm flex flex-col items-center justify-center gap-4 p-6 text-center">
          ...
          <div>
            <p className="text-white font-semibold text-lg mb-1">Aperçu terminé</p>
            <p className="text-noir-400 text-sm">Achetez pour regarder la vidéo complète</p>
          </div>
        </div>
      )}
      {apercuDuree && !blocked && (
        <div className="absolute bottom-3 right-3 bg-noir-950/80 backdrop-blur text-xs text-gold-400 px-2 py-1 rounded-full">
          Aperçu {apercuDuree}s
        </div>
      )}
    </div>
  )
...
// ── Lecteur audio avec extrait limité ──────────────────────────────────────
  ...
  useEffect(() => {
    ...
    const handleTime = () => {
      ...
      if (audio.currentTime >= maxTime) {
        audio.pause(); setBlocked(true); setPlaying(false)
      }
    }
      ...
      <div className="flex items-center gap-4 mb-4">
        <button onClick={togglePlay} disabled={blocked}
          className={`w-12 h-12 rounded-full flex items-center justify-center transition-all ${
            blocked ? 'bg-noir-700 text-noir-500 cursor-not-allowed' : 'bg-gold-500 text-noir-950 hover:bg-gold-400'
          }`}>
          {playing
        ...
        <div className="flex-1">
          ...
          <div className="flex justify-between text-xs text-noir-500 mt-1">
            ...
            <span>Aperçu {maxTime}s</span>
          </div>
        </div>
      </div>
      {blocked && (
        <div className="bg-noir-900 border border-gold-500/20 rounded-xl p-4 text-center">
          <p className="text-white text-sm font-semibold mb-1">Aperçu terminé</p>
          <p className="text-noir-400 text-xs">Achetez pour écouter l'audio complet</p>
        </div>
      )}
    </div>
  )
...
// ── Aperçu PDF — N pages via iframe scrollable ─────────────────────────────
function PdfViewer({ url, apercuPages, nbPages }: { url: string; apercuPages?: number | null; nbPages?: number | null }) {
  ...
  // Utiliser PDF.js via CDN pour afficher page par page
  // On utilise l'URL directe avec #page=N pour les PDFs hébergés
  const isCloudinary = url.includes('cloudinary.com')
  const isPdfUrl = url.toLowerCase().includes('.pdf') || isCloudinary
      ...
      <div className="space-y-3">
        <div className="bg-noir-800 rounded-xl p-3 text-center">
          <p className="text-gold-400 text-xs font-semibold uppercase tracking-wider">
            Aperçu — {pages} premières pages sur {total}
          </p>
        </div>
        <div className="rounded-xl overflow-hidden bg-noir-900 border border-noir-700" style={{ height: '500px' }}>
          <iframe src={viewerUrl} className="w-full h-full" title="Aperçu PDF" />
        </div>
        <p className="text-noir-600 text-xs text-center">Aperçu limité. Achetez pour accéder au document complet ({total} pages).</p>
      </div>
    )
  }

  // Pour les PDFs directs : afficher avec iframe + navigation par page
  // On utilise le paramètre #page= pour naviguer
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="rounded-xl overflow-hidden bg-noir-900 border border-noir-700 relative" style={{ height: '480px' }}>
        ...
        />
        {/* Overlay pour empêcher le téléchargement via clic droit */}
        <div className="absolute inset-0 pointer-events-none" style={{ userSelect: 'none' }} />
      </div>

      {/* Message blocage après dernière page */}
      {currentPage >= pages && (
        <div className="bg-gradient-to-r from-gold-500/10 to-noir-900 border border-gold-500/20 rounded-xl p-4 text-center">
          ...
          <p className="text-noir-400 text-xs">
            Ce document contient {total} pages au total. Achetez pour accéder à l'intégralité.
          </p>
        </div>
      )}

      <p className="text-noir-600 text-xs text-center">
        Téléchargement et impression désactivés pour cet aperçu.
      </p>
    </div>
  )
}

// ── Lecteur Cloudinary (vidéo/audio) ──────────────────────────────────────
function CloudinaryViewer({ url, type, apercuDuree }: { url: string; type: string; apercuDuree?: number | null }) {
  if (type === 'audio') {
    return <AudioPlayer url={url} apercuDuree={apercuDuree} />
  }
  // Vidéo Cloudinary
  return (
...
export default function ApercuModal({ ressource, onClose, onBuy }: ApercuModalProps) {
  const isYoutube = !!(ressource.youtube_url && (ressource.youtube_url.includes('youtube') || ressource.youtube_url.includes('youtu.be')))
  const isCloudinaryApercu = !!(ressource.apercu_url && ressource.apercu_url.includes('cloudinary'))
  const hasAudio = !!(ressource.apercu_url && ressource.type === 'audio')
  const hasPdf = !!(ressource.type === 'documentation' && (ressource.apercu_url || ressource.fichier_url))
      ...
      <motion.div
        ...
        <motion.div
          ...
          <div className="flex items-center justify-between px-5 py-4 border-b border-noir-800 shrink-0">
            <div>
              ...
              <p className="text-gold-500 text-xs mt-0.5">
                Aperçu gratuit
                {hasPdf && ressource.apercu_pages && ` — ${ressource.apercu_pages} pages sur ${ressource.nb_pages || '?'}`}
                {(isYoutube || hasAudio) && ressource.apercu_duree && ` — ${ressource.apercu_duree}s`}
              </p>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-5">
            ...
            {/* Cloudinary (audio ou vidéo) */}
            {!isYoutube && isCloudinaryApercu && (
              <CloudinaryViewer url={ressource.apercu_url!} type={ressource.type} apercuDuree={ressource.apercu_duree} />
            )}

            {/* Audio avec URL aperçu non-Cloudinary */}
            {!isYoutube && !isCloudinaryApercu && hasAudio && (
              <AudioPlayer url={ressource.apercu_url!} apercuDuree={ressource.apercu_duree} />
            )}

            {/* PDF */}
            {!isYoutube && !isCloudinaryApercu && !hasAudio && hasPdf && (
              <PdfViewer
                url={ressource.apercu_url || ressource.fichier_url!}
                apercuPages={ressource.apercu_pages}
          ...
          <div className="px-5 py-4 border-t border-noir-800 shrink-0 flex items-center justify-between gap-3">
            <div>
              <span className="text-white font-bold text-lg">
                {ressource.est_gratuit ? 'Gratuit' : `${ressource.prix} €`}
              </span>
