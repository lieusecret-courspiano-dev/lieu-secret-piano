...
export default function PublicNav({ settings }: { settings?: Settings }) {
  ...
  const socialLinks = [
    settings?.instagram && { href: settings.instagram, label: 'Instagram', icon: <svg width="15" height="15" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/></svg> },
    settings?.youtube && { href: settings.youtube, label: 'YouTube', icon: <svg width="15" height="15" fill="currentColor" viewBox="0 0 24 24"><path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg> },
    settings?.tiktok && { href: settings.tiktok, label: 'TikTok', icon: <TikTokIcon size={15} /> },
  ].filter(Boolean) as { href: string; label: string; icon: React.ReactNode }[]

  const bannerActive = settings?.banner_actif === 'true' && settings?.banner_message
  const bannerColor = settings?.banner_type === 'urgent' ? 'bg-red-600' : settings?.banner_type === 'success' ? 'bg-green-600' : 'bg-gold-600'

  return (
    <>
      {bannerActive && (
        <div className={`${bannerColor} text-white text-center text-sm py-2 px-4 font-medium z-50 relative`}>
      ...
      <motion.header
        ...
        className={`fixed left-0 right-0 z-50 transition-all duration-300 ${bannerActive ? 'top-9' : 'top-0'} ${
          scrolled ? 'bg-noir-950/95 backdrop-blur-xl border-b border-noir-800/60 shadow-2xl shadow-black/20' : 'bg-transparent'
        }`}
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16 lg:h-20">
            ...
            <div className="hidden lg:flex items-center gap-2">
              {socialLinks.map(s => (
                <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer"
                  className="text-noir-500 hover:text-gold-400 transition-colors p-1.5 rounded-lg hover:bg-noir-800" aria-label={s.label}>
                  {s.icon}
                </a>
              ))}
              {socialLinks.length > 0 && <div className="w-px h-5 bg-noir-800" />}
              <ThemeToggle />
              <Link href="/espace-eleve/login" className="text-sm text-noir-400 hover:text-white transition-colors px-3 py-2 rounded-xl hover:bg-noir-800">
                Connexion
              </Link>
              <Link href="/reservation" className="btn-gold text-sm px-5 py-2.5 shadow-lg shadow-gold-500/20">
                Réserver
              </Link>
            </div>
            ...
            <button onClick={() => setMenuOpen(!menuOpen)}
              className="lg:hidden p-2 rounded-xl text-noir-400 hover:text-white hover:bg-noir-800 transition-colors" aria-label="Menu">
            ...
            <motion.div
              ...
              <div className="px-4 py-4 space-y-1">
                  ...
                  <motion.div key={link.href} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}>
                    <Link href={link.href}
                      className={`block px-4 py-3 rounded-xl text-sm font-medium transition-colors ${
                        link.highlight ? 'bg-gold-500/10 text-gold-400 border border-gold-500/20'
                        : pathname === link.href ? 'bg-noir-800 text-white'
                ...
                <div className="pt-2 flex items-center gap-3">
                  <ThemeToggle />
                  <Link href="/espace-eleve/login" className="text-sm text-noir-400 hover:text-white transition-colors">Connexion</Link>
                  <Link href="/reservation" className="btn-gold text-sm px-4 py-2 flex-1 text-center">Réserver</Link>
                </div>
              </div>
            </motion.div>
          )}
