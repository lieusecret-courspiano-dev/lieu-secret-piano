...
export async function GET(req: NextRequest) {
  ...
  if (id) {
    ...
    const { data: resultats } = await supabaseAdmin
      .from('quiz_resultats').select('*')
      .eq('quiz_id', id).eq('eleve_id', eleve.id)
  ...
  const { data: allQuiz } = await supabaseAdmin
    .from('quiz').select('*, quiz_questions(count)').order('created_at', { ascending: true })
...
export async function POST(req: NextRequest) {
  ...
  for (const q of questions) {
    ...
    corrections[q.id] = { correct, bonne_reponse: q.bonne_reponse || '', explication: q.explication, question: q.question }
  }
