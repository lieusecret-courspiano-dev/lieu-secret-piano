...
export async function POST(req: NextRequest) {
  ...
  const { pack_label, heures_total, heures_utilisees, montant, acheteur_nom, acheteur_email, payment_method, date_paiement } = await req.json()
  ...
  const heuresUtilisees = parseInt(String(heures_utilisees || 0))
  const heuresTotal     = parseInt(String(heures_total))
  const heuresRestantes = Math.max(0, heuresTotal - heuresUtilisees)
