...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const { data: packs } = await supabaseAdmin
      .from('course_packs')
      .select('id, montant, heures_total, heures_restantes, status, payment_method, created_at')
      .gte('created_at', fromISO).lte('created_at', toISO)
    ...
    const { data: giftCards } = await supabaseAdmin
      .from('gift_cards')
      .select('id, montant, status, payment_method, created_at')
      .gte('created_at', fromISO).lte('created_at', toISO)
      .neq('status', 'pending_virement')
    ...
    const monthlyData = months.map(month => {
      const mPacks  = (packs || []).filter(p => p.created_at?.startsWith(month))
      const mGifts  = (giftCards || []).filter(g => g.created_at?.startsWith(month))
