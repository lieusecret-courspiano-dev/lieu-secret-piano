...
const COURS_TYPES = [
  {
    ...
    features: [
      ...
      'Enregistrement disponible sur demande',
    ],
  ...
  {
    icon: '🎵',
    titre: 'Atelier de groupe',
    sous: 'Jusqu\'à 8 participants',
    ...
    features: [
      'Thème défini à l\'avance',
      'Échanges et partage entre élèves',
      'Ambiance conviviale et motivante',
      'Tarif accessible',
      'Gratuit ou payant selon l\'atelier',
      'Replay disponible',
    ],
  ...
  },
  {
    ...
    features: [
      ...
      'Petits groupes (6 participants max)',
...
export default function CoursPage() {
  ...
  return (
    <div className="min-h-screen bg-noir-950 text-noir-100">
      ...
      <section className="pt-32 pb-20 px-4 text-center relative overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            <p className="text-noir-300 text-lg max-w-2xl mx-auto leading-relaxed mb-8">
              Cours individuels, ateliers collectifs ou masterclass — choisissez la formule qui correspond à vos objectifs et votre style d'apprentissage.
            </p>
            <Link href="/essai" className="btn-gold text-base px-8 py-4 shadow-xl shadow-gold-500/20">
          ...
          <div className="grid lg:grid-cols-3 gap-6">
            {COURS_TYPES.map((cours, i) => (
              <FadeUp key={i} delay={i * 0.1}>
                <div className="bg-noir-900 border border-noir-800 rounded-2xl p-7 hover:border-noir-700 transition-all hover:-translate-y-1 h-full flex flex-col">
                  <div className="text-4xl mb-4">{cours.icon}</div>
                  <span className="text-gold-500 text-xs font-semibold uppercase tracking-widest mb-2 block">{cours.sous}</span>
                  <h2 className="font-serif text-2xl text-white mb-3">{cours.titre}</h2>
      ...
      {/* Niveaux */}
      <section className="py-20 px-4 bg-noir-900/30">
        <div className="max-w-5xl mx-auto">
          <FadeUp className="text-center mb-12">
            <span className="text-gold-500 text-xs font-semibold uppercase tracking-widest">Niveaux</span>
            <h2 className="font-serif text-4xl text-white mt-3">Pour tous les niveaux</h2>
          </FadeUp>
          ...
          <motion.div key={activeNiveau} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}
            className="bg-noir-900 border border-gold-500/20 rounded-2xl p-8">
            <h3 className="font-serif text-2xl text-white mb-3">{NIVEAUX[activeNiveau].titre}</h3>
            <p className="text-noir-300 mb-6">{NIVEAUX[activeNiveau].desc}</p>
      ...
      <section className="py-20 px-4 text-center">
        <FadeUp>
          <h2 className="font-serif text-4xl text-white mb-4">Prêt à commencer ?</h2>
          <p className="text-noir-400 mb-8">Premier cours d'essai gratuit — sans engagement.</p>
