...
// Contenu par défaut pour les articles statiques
const DEFAULT_CONTENT: Record<string, { titre: string; contenu: string; categorie: string; temps_lecture: string }> = {
  'apprendre-piano-adulte': {
    ...
    contenu: `Beaucoup de personnes pensent qu'il est trop tard pour apprendre le piano une fois adulte. Cette idée reçue décourage de nombreux passionnés de musique. Pourtant, la réalité est bien différente.
...
Il n'est jamais trop tard pour commencer le piano. La seule condition est de le vouloir vraiment.`,
  ...
  'choisir-clavier-piano': {
    ...
    contenu: `Le choix de votre premier instrument est crucial pour votre apprentissage. Voici les critères essentiels à considérer.
...
N'oubliez pas d'inclure une pédale de sustain dans votre budget. Elle est essentielle dès les premières semaines d'apprentissage.`,
  ...
  'progresser-piano-en-ligne': {
    ...
    contenu: `Les cours de piano en ligne offrent une flexibilité incomparable. Voici comment en tirer le meilleur parti.
    ...
    fetch(`/api/blog?slug=${encodeURIComponent(slug)}`)
      ...
      .then(data => {
        if (data && data.titre) {
          ...
          // Chercher dans les articles par défaut
          const def = DEFAULT_CONTENT[slug]
          if (def) {
            setArticle({
              id: slug, slug, extrait: '', image_url: null,
      ...
      <section className="pt-28 sm:pt-32 pb-10 px-4">
        <div className="max-w-3xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            {/* Breadcrumb */}
            <div className="flex items-center gap-2 text-sm text-noir-500 mb-8">
              ...
              <span>/</span>
              <span className="text-noir-400 truncate">{article.titre}</span>
            </div>
            ...
            <div className="flex items-center gap-3 mb-5 flex-wrap">
              ...
              <span className="text-noir-500 text-sm">{new Date(article.created_at).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' })}</span>
        ...
        <div className="max-w-3xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.3 }}
            ...
            {paragraphs.map((para, i) => {
              if (para.startsWith('## ')) {
                return (
                  <h2 key={i} className="font-serif text-2xl sm:text-3xl text-white mt-10 mb-4 first:mt-0">
                    {para.replace('## ', '')}
                  </h2>
                )
              }
              if (para.startsWith('# ')) {
                return (
                  <h1 key={i} className="font-serif text-3xl sm:text-4xl text-white mt-10 mb-4 first:mt-0">
                    {para.replace('# ', '')}
                  </h1>
                )
              }
              return (
                <p key={i} className="text-noir-300 leading-relaxed text-base sm:text-lg">
                  {para}
                </p>
              )
            })}
