...
interface Temoignage {
  id: string; nom: string; note: number; commentaire: string; created_at: string
  est_publie: boolean; instrument?: string
}
...
const FAQ_ITEMS = [
  ...
  { q: "Comment se déroulent les cours en ligne ?", a: "Les cours se déroulent via Zoom ou Google Meet. Vous recevez le lien par email après réservation. Une connexion internet stable et un clavier/piano suffisent." },
...
const STEPS = [
  ...
  { n: '02', title: 'Rencontrez votre prof', desc: 'Lors du premier cours, nous évaluons votre niveau et définissons ensemble vos objectifs musicaux.' },
  ...
  return (
    <div className="min-h-screen bg-noir-950 text-noir-100 overflow-x-hidden">
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 text-center pt-24">
          ...
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.2 }}
            ...
            {s?.hero_subtitle || 'Cours individuels, ateliers de groupe et masterclass en ligne. Une pédagogie bienveillante pour tous les niveaux.'}
          </motion.p>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.3 }}
            ...
            <Link href="/essai"
              className="group relative inline-flex items-center gap-2 bg-gold-500 hover:bg-gold-400 text-noir-950 font-semibold px-8 py-4 rounded-2xl transition-all duration-200 text-base shadow-2xl shadow-gold-500/30 hover:shadow-gold-500/50 hover:-translate-y-0.5">
              ...
              <motion.span className="ml-1" animate={{ x: [0, 4, 0] }} transition={{ duration: 1.5, repeat: Infinity }}>→</motion.span>
            </Link>
            <Link href="/reservation"
              className="inline-flex items-center gap-2 border border-noir-700 hover:border-gold-500/50 text-noir-300 hover:text-white px-8 py-4 rounded-2xl transition-all duration-200 text-base hover:bg-noir-800/50">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8, delay: 0.5 }}
            ...
            {[
              ...
              { val: '🌍', label: s?.stats_label3 || 'Monde entier' },
              { val: '★', label: s?.stats_label4 || 'Tous niveaux' },
            ].map((stat, i) => (
              <div key={i} className="bg-noir-900/60 backdrop-blur border border-noir-800/60 rounded-2xl px-4 py-4 text-center">
                <div className="text-2xl font-bold text-gold-400 mb-1">{stat.val}</div>
      ...
      {featuredEvent && (
        <section className="py-8 px-4">
          <div className="max-w-4xl mx-auto">
            <FadeUp>
              <div className="relative bg-gradient-to-r from-gold-500/10 to-gold-600/5 border border-gold-500/30 rounded-2xl p-6 overflow-hidden">
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <h3 className="text-white font-semibold text-lg mt-0.5">{featuredEvent.title}</h3>
                    {featuredEvent.description && <p className="text-noir-400 text-sm mt-1 line-clamp-2">{featuredEvent.description}</p>}
                  </div>
                  <Link href={`/reservation?tab=evenements`}
                    className="btn-gold text-sm px-5 py-2.5 shrink-0">
      ...
      <section className="py-24 px-4">
        <div className="max-w-6xl mx-auto">
          <FadeUp className="text-center mb-16">
            ...
            <h2 className="font-serif text-4xl md:text-5xl text-white mt-3">{s?.steps_titre || 'Comment ça marche ?'}</h2>
          </FadeUp>
            ...
            {STEPS.map((step, i) => (
              <FadeUp key={i} delay={i * 0.1}>
                <div className="relative group">
                  ...
                  <div className="bg-noir-900 border border-noir-800 rounded-2xl p-6 hover:border-gold-500/30 transition-all duration-300 hover:-translate-y-1 h-full">
                    <div className="w-12 h-12 rounded-xl bg-gold-500/10 border border-gold-500/20 flex items-center justify-center mb-4 group-hover:bg-gold-500/20 transition-colors">
                      <span className="font-serif text-gold-400 text-lg font-bold">{step.n}</span>
                    </div>
      ...
      {/* ══════════════════════════════════════════
          OFFRES / COURS
      ══════════════════════════════════════════ */}
      <section id="offres" className="py-24 px-4 bg-noir-900/30">
        <div className="max-w-6xl mx-auto">
          <FadeUp className="text-center mb-16">
            <span className="text-gold-500 text-xs font-semibold uppercase tracking-widest">Nos formules</span>
            <h2 className="font-serif text-4xl md:text-5xl text-white mt-3">{s?.offres_titre || 'Choisissez votre formule'}</h2>
            {s?.offres_sous_titre && <p className="text-noir-400 mt-4 max-w-xl mx-auto">{s.offres_sous_titre}</p>}
          </FadeUp>
            ...
            {[
              {
                ...
                desc: s?.offre1_desc || 'Un cours dédié entièrement à vous.',
              ...
              },
              {
                ...
                desc: s?.offre2_desc || 'Apprenez en groupe dans une ambiance conviviale.',
              ...
              },
              {
                ...
                desc: s?.offre3_desc || 'Poussez votre technique plus loin.',
                ...
                <div className={`relative flex flex-col h-full rounded-2xl border p-6 transition-all duration-300 hover:-translate-y-1 ${
                  ...
                  {offre.popular && (
                    <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                      <span className="bg-gold-500 text-noir-950 text-xs font-bold px-3 py-1 rounded-full">Populaire</span>
                    </div>
                  )}
                  <div className="text-3xl mb-4">{offre.icon}</div>
                  <div className="mb-1">
                    <span className="text-gold-500 text-xs font-semibold uppercase tracking-widest">{offre.sous}</span>
                  </div>
                  <h3 className="font-serif text-2xl text-white mb-2">{offre.titre}</h3>
                  <p className="text-noir-400 text-sm mb-6 leading-relaxed">{offre.desc}</p>
                  <ul className="space-y-2.5 mb-8 flex-1">
                    {offre.features.map((f, j) => (
                      <li key={j} className="flex items-start gap-2.5 text-sm text-noir-300">
                        <svg width="16" height="16" fill="none" stroke="#f59e0b" strokeWidth="2" viewBox="0 0 24 24" className="shrink-0 mt-0.5"><polyline points="20 6 9 17 4 12"/></svg>
                        {f}
                      </li>
                    ))}
                  </ul>
          ...
          <div className="max-w-6xl mx-auto">
            <div className="grid lg:grid-cols-2 gap-12 items-center">
              <FadeIn>
                <div className="relative">
                  <div className="aspect-[4/5] rounded-3xl overflow-hidden bg-noir-900 border border-noir-800">
                    {s?.prof_photo ? (
                      <Image src={s.prof_photo} alt={s.prof_nom || 'Professeur'} fill className="object-cover" />
                    ) : (
                      <div className="w-full h-full flex items-center justify-center">
                        <svg width="80" height="80" fill="none" stroke="#3a3a5c" strokeWidth="1" viewBox="0 0 24 24">
                  ...
                  {/* Floating card */}
                  <div className="absolute -bottom-6 -right-6 bg-noir-900 border border-gold-500/30 rounded-2xl p-4 shadow-xl">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="text-white text-sm font-semibold">Pédagogie bienveillante</div>
                        <div className="text-noir-400 text-xs">Adaptée à chaque élève</div>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <FadeUp delay={0.2}>
                ...
                <h2 className="font-serif text-4xl md:text-5xl text-white mt-3 mb-2">{s?.prof_nom || 'Votre professeur'}</h2>
                {s?.prof_titre && <p className="text-gold-400 text-sm mb-6">{s.prof_titre}</p>}
                {s?.prof_bio && <p className="text-noir-300 leading-relaxed mb-6">{s.prof_bio}</p>}
                {s?.prof_vision && (
                  <div className="bg-noir-900/60 border border-noir-800 rounded-2xl p-5 mb-6">
                    <h4 className="text-gold-400 text-sm font-semibold mb-2 uppercase tracking-wider">Vision</h4>
                    <p className="text-noir-300 text-sm leading-relaxed">{s.prof_vision}</p>
                  </div>
                )}
                {s?.prof_pedagogie && (
                  <div className="bg-noir-900/60 border border-noir-800 rounded-2xl p-5 mb-8">
                    <h4 className="text-gold-400 text-sm font-semibold mb-2 uppercase tracking-wider">Pédagogie</h4>
      ...
      {/* ══════════════════════════════════════════
          TARIFS
      ══════════════════════════════════════════ */}
      <section id="tarifs" className="py-24 px-4 bg-noir-900/30">
        <div className="max-w-5xl mx-auto">
          <FadeUp className="text-center mb-16">
            <span className="text-gold-500 text-xs font-semibold uppercase tracking-widest">Investissement</span>
            <h2 className="font-serif text-4xl md:text-5xl text-white mt-3">Tarifs transparents</h2>
            <p className="text-noir-400 mt-4">Sans engagement, sans frais cachés.</p>
          </FadeUp>

          {/* Cours à l'unité */}
          <FadeUp className="mb-8">
            <div className="bg-noir-900 border border-noir-800 rounded-2xl p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
              <div>
                <h3 className="text-white font-semibold text-lg">Cours individuel — 1 heure</h3>
                <p className="text-noir-400 text-sm mt-1">Cours en tête-à-tête via Zoom ou Google Meet</p>
              </div>
              <div className="flex items-center gap-4">
                <div className="text-right">
                  <div className="text-3xl font-bold text-white">{s?.tarif_cours_1h || '22'} €</div>
                  <div className="text-noir-500 text-xs">par heure</div>
                </div>
                <Link href="/reservation" className="btn-gold">Réserver</Link>
              </div>
            </div>
          </FadeUp>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {packs.map((pack, i) => (
                <FadeUp key={i} delay={i * 0.08}>
                  <div className={`relative rounded-2xl border p-5 text-center transition-all hover:-translate-y-1 ${
                    pack.popular
                    ...
                    <div className="text-3xl font-bold text-gold-400 my-3">{pack.prix} €</div>
                    {pack.desc && <p className="text-noir-400 text-xs mb-4">{pack.desc}</p>}
                    <Link href="/packs" className={`block py-2 rounded-xl text-sm font-medium transition-all ${
          ...
          <FadeUp className="mt-8 text-center">
            <p className="text-noir-500 text-sm">
              Cours d'essai gratuit disponible •{' '}
        ...
        <section className="py-24 px-4">
          <div className="max-w-5xl mx-auto">
            <FadeUp className="text-center mb-16">
              ...
              <h2 className="font-serif text-4xl md:text-5xl text-white mt-3">Ce que disent nos élèves</h2>
            </FadeUp>
            ...
            <FadeIn className="mb-8">
              <AnimatePresence mode="wait">
                <motion.div key={testimonialIdx}
                  ...
                  className="bg-gradient-to-br from-noir-900 to-noir-900/50 border border-gold-500/20 rounded-3xl p-8 md:p-10 text-center">
                  <div className="flex justify-center mb-4">
                    {[...Array(5)].map((_, i) => (
                      <svg key={i} width="20" height="20" fill={i < (temoignages[testimonialIdx]?.note || 5) ? '#f59e0b' : '#3a3a5c'} viewBox="0 0 24 24">
                        <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/>
                      </svg>
                    ))}
                  </div>
                  <blockquote className="text-white text-lg md:text-xl font-serif leading-relaxed mb-6 max-w-2xl mx-auto">
                    "{temoignages[testimonialIdx]?.commentaire}"
                  </blockquote>
                  <div className="text-gold-400 font-semibold">{temoignages[testimonialIdx]?.nom}</div>
                </motion.div>
              </AnimatePresence>
            </FadeIn>
            ...
            <div className="flex justify-center gap-2 mb-10">
              {temoignages.map((_, i) => (
                <button key={i} onClick={() => setTestimonialIdx(i)}
                  className={`w-2 h-2 rounded-full transition-all ${i === testimonialIdx ? 'bg-gold-500 w-6' : 'bg-noir-700 hover:bg-noir-600'}`} />
              ))}
            </div>
            ...
            <div className="grid md:grid-cols-3 gap-4">
              {temoignages.slice(0, 3).map((t, i) => (
                <FadeUp key={t.id} delay={i * 0.1}>
                  <div className="bg-noir-900 border border-noir-800 rounded-2xl p-5 hover:border-noir-700 transition-all">
                    <div className="flex mb-3">
                      {[...Array(5)].map((_, j) => (
                        <svg key={j} width="14" height="14" fill={j < t.note ? '#f59e0b' : '#3a3a5c'} viewBox="0 0 24 24">
                          <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/>
                        </svg>
                      ))}
                    </div>
                    <p className="text-noir-300 text-sm leading-relaxed mb-4 line-clamp-4">"{t.commentaire}"</p>
      ...
      {/* ══════════════════════════════════════════
          FAQ
      ══════════════════════════════════════════ */}
      <section className="py-24 px-4 bg-noir-900/30">
        <div className="max-w-3xl mx-auto">
          <FadeUp className="text-center mb-16">
            <span className="text-gold-500 text-xs font-semibold uppercase tracking-widest">Questions fréquentes</span>
            <h2 className="font-serif text-4xl md:text-5xl text-white mt-3">FAQ</h2>
          </FadeUp>
              ...
              <FadeUp key={i} delay={i * 0.05}>
                <div className={`border rounded-2xl overflow-hidden transition-all ${
                  ...
                  <button onClick={() => setFaqOpen(faqOpen === i ? null : i)}
                    ...
                    <span className={`font-medium text-sm ${faqOpen === i ? 'text-white' : 'text-noir-200'}`}>{item.q}</span>
                    <motion.span animate={{ rotate: faqOpen === i ? 45 : 0 }} transition={{ duration: 0.2 }}
                      className={`shrink-0 ml-4 ${faqOpen === i ? 'text-gold-400' : 'text-noir-500'}`}>
                  ...
                  <AnimatePresence>
                    {faqOpen === i && (
                      <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }}
                        ...
                        <div className="px-6 pb-5 text-noir-400 text-sm leading-relaxed border-t border-noir-800 pt-4">
                          {item.a}
                        </div>
                      </motion.div>
                    )}
      ...
      <section className="py-24 px-4">
        <div className="max-w-4xl mx-auto">
          <FadeUp>
            <div className="relative bg-gradient-to-br from-gold-500/10 via-noir-900 to-noir-900 border border-gold-500/20 rounded-3xl p-10 md:p-16 text-center overflow-hidden">
              ...
              <div className="relative">
                ...
                <p className="text-noir-300 text-lg mb-8 max-w-xl mx-auto">
                  {s?.cta_sous_titre || 'Réservez votre premier cours dès maintenant. Premier cours d\'essai offert.'}
                </p>
                <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                  <Link href="/essai"
                    className="btn-gold text-base px-8 py-4 shadow-2xl shadow-gold-500/30">
                    Cours d'essai gratuit
                  </Link>
                  <Link href="/reservation"
                    className="border border-noir-700 hover:border-gold-500/50 text-noir-300 hover:text-white px-8 py-4 rounded-2xl text-base transition-all">
                    Voir les créneaux
                  </Link>
                </div>
              </div>
